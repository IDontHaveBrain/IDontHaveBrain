...
## 💼 소개
분산 시스템 설계, 복잡한 기술적 문제 해결에 전문성을 가진 백엔드 개발자입니다. 기술적 도전을 즐기며 가독성, 확장성, 유지보수성을 모두 고려한 소프트웨어 개발에 주력합니다. **Java, Spring, JPA 기반 백엔드 아키텍처 설계 및 구현에 전문성을 보유**하고 있으며, 새로운 기술적 도전을 적극적으로 수용하며, 복잡한 레거시 시스템을 현대적인 아키텍처로 전환하고, 지속적 통합/배포 파이프라인을 구축하는 경험을 쌓아왔습니다.
...
- **tree-sitter-language-pack:**
    ...
    - 핵심 수정 내용:
        - 경로 처리 정규식 패턴을 개선하여 Windows와 Unix 경로 모두 지원
        - Windows 특화 tree-sitter CLI 명령 실행 방식 추가
        - Windows에서 C 소스 파일 include 경로 처리 최적화
        - MSVC 컴파일러 옵션 추가(UTF-8 강제 인코딩)로 비-영어 로케일 지원
    - 추가 기여:
